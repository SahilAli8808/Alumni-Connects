...
router.post('/insertAlumniExcel', 
    ...
    parseExcel,
    (req, res) => {
        console.log('Parsed Data:', req.data); // Check the received data
        console.log('College Info:', req.college); // Verify college information

        req.data.forEach((alumni) => {
            // Log each alumni entry
            console.log('Processing Alumni:', alumni);
            ...
            // Required fields
            alumni["firstName"] = alumni.firstName || "First Name"; // Replace with actual logic if needed
            alumni["lastName"] = alumni.lastName || "Last Name"; // Optional, but ensure it's handled
            alumni["rollNumber"] = alumni.rollNumber || "Roll Number"; // Replace with actual logic
            alumni["branch"] = alumni.branch || "Branch"; // Replace with actual logic
            alumni["password"] = alumni.password || "defaultPassword"; // Replace with actual logic
        ...
        Alumni.insertMany(req.data, {
            ordered: true,
            rawResult: true
        })
        .then((result) => {
            console.log('Insertion Result:', result); // Check the result
            res.send(result);
        })
        .catch((err) => {
            console.error('Insertion Error:', err); // Log the error
